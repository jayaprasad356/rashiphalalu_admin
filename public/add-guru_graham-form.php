...
if (isset($_POST['btnAdd'])) {
       ...
       if (!empty($year) && !empty($rasi)) {
                ...
                $sql_query = "INSERT INTO guru_graham_tab (year,rasi)VALUES('$year','$rasi')";
                $db->sql($sql_query);
                $result = $db->getResult();
                if (!empty($result)) {
                    $result = 0;
                ...
                if ($result == 1) {
                    $sql = "SELECT id FROM guru_graham_tab ORDER BY id DESC LIMIT 1";
                    $db->sql($sql);
                    $res = $db->getResult();
                    $guru_graham_tab_id = $res[0]['id'];
                    for ($i = 0; $i < count($_POST['title']); $i++) {

                        $title = $db->escapeString(($_POST['title'][$i]));
                        $description = $db->escapeString(($_POST['description'][$i]));
                        $sql = "INSERT INTO guru_graham_tab_variant (guru_graham_tab_id,title,description) VALUES('$guru_graham_tab_id','$title','$description')";
                        $db->sql($sql);
...
<section class="content-header">
    <h1>Add Guru Graham<small><a href='guru_graham.php'> <i class='fa fa-angle-double-left'></i>&nbsp;&nbsp;&nbsp;Back to Guru Graham</a></small></h1>
    ...
    <div class="row">
        <div class="col-md-10">
            ...
            <div class="box box-primary">
                ...
                <!-- /.box-header -->
                <!-- form start -->
                <form name="add_guru_graham_form" method="post" enctype="multipart/form-data">
                    <div class="box-body">
                           <div class="row">
