...
if (isset($_POST['btnAdd'])) {
    ...
    $rasi = $db->escapeString(($_POST['rasi']));
       ...
       if (!empty($title) && !empty($description) && !empty($rasi)) {
                ...
                $sql_query = "INSERT INTO janma_rashulu_tab (title,description,rasi)VALUES('$title','$description','$rasi')";
                $db->sql($sql_query);
                $result = $db->getResult();
                if (!empty($result)) {
                    $result = 0;
                ...
                if ($result == 1) {
                    $sql = "SELECT id FROM janma_rashulu_tab ORDER BY id DESC LIMIT 1";
                    $db->sql($sql);
                    $res = $db->getResult();
                    $janma_rashulu_tab_id = $res[0]['id'];
                    for ($i = 0; $i < count($_POST['title_description']); $i++) {
                        ...
                        $sql = "INSERT INTO janma_rashulu_tab_variant (janma_rashulu_tab_id,title_description,title_description1) VALUES('$janma_rashulu_tab_id','$title_description','$title_description1')";
                        $db->sql($sql);
                    ...
                    $error['add_grahalu_tab'] = "<section class='content-header'>
                                                    <span class='label label-success'>Janma Rashulu Vati Swabhavalu Added Successfully</span> </section>";
                } else {
...
<section class="content-header">
    <h1>Add Janma Rashulu Vati Swabhavalu<small><a href='janma_rashulu.php'> <i class='fa fa-angle-double-left'></i>&nbsp;&nbsp;&nbsp;Back to Janma Rashulu Vati Swabhavalu</a></small></h1>
