...
if (isset($_GET['id'])) {
	...
	$category_data = array();
	$sql = "select id,name from shani_graham order by id asc";
	$db->sql($sql);
...
if (isset($_POST['btnEdit'])) {
   ...
   if (!empty($year) && !empty($rasi)) {
				$sql_query = "UPDATE shani_graham_tab SET year='$year',rasi='$rasi' WHERE id =$ID";
				$db->sql($sql_query);
				$res = $db->getResult();
			...
			if ($update_result == 1)
			{
				for ($i = 0; $i < count($_POST['title']); $i++) {
					$shani_graham_tab_id = $db->escapeString(($_POST['shani_graham_tab_variant_id'][$i]));
					$title = $db->escapeString(($_POST['title'][$i]));
					$description = $db->escapeString(($_POST['description'][$i]));
					$sql = "UPDATE shani_graham_tab_variant SET title='$title',description='$description' WHERE id =$shani_graham_tab_id";
					$db->sql($sql);
				...
				if (
					...
					for ($i = 0; $i < count($_POST['insert_title']); $i++) {
						...
						if (!empty($title) || !empty($description)) {
							$sql = "INSERT INTO shani_graham_tab_variant (shani_graham_tab_id,title,description) VALUES('$ID','$title','$description')";
							$db->sql($sql);

						}
					}
				}
					$error['update_grahalutab'] = " <section class='content-header'><span class='label label-success'>Shani Graham updated Successfully</span></section>";
			} else {
...
$sql_query = "SELECT * FROM shani_graham_tab WHERE id =" . $ID;
$db->sql($sql_query);
$res = $db->getResult();

$sql_query = "SELECT * FROM shani_graham_tab_variant WHERE shani_graham_tab_id =" . $ID;
$db->sql($sql_query);
$resslot = $db->getResult();

if (isset($_POST['btnCancel'])) { ?>
	<script>
		window.location.href = "shani_graham_tab.php";
	</script>
<?php } ?>
<section class="content-header">
	<h1>
		Edit Shani Graham<small><a href='shani_graham.php'><i class='fa fa-angle-double-left'></i>&nbsp;&nbsp;&nbsp;Back to Shani Graham</a></small></h1>
					...
					<div class="row">
						   ...
						   <div id="variations">
							...
							foreach ($resslot as $row) {
								...
								<div id="packate_div">
									<div class="row">
									    <input type="hidden" class="form-control" name="shani_graham_tab_variant_id[]" id="shani_graham_tab_variant_id" value='<?= $row['id']; ?>' />
									    <div class="col-md-4">
    ...
    $(document).on('click', '.remove_variation', function() {
        if ($(this).data('id') == 'data_delete') {
            if (confirm('Are you sure? Want to delete this row')) {
                var id = $(this).closest('div.row').find("input[id='shani_graham_tab_variant_id']").val();
                $.ajax({
